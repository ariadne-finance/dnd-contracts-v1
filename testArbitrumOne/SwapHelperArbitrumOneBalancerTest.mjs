...
describe("SwapHelperArbitrumOneBalancer", function() {
  let snapshot;
  ...
  before(async () => {
    ...
    const SwapHelper = await ethers.getContractFactory('SwapHelperArbitrumOneBalancer');
  ...
  it("swap usdce to wsteth and back", async () => {
    ...
    expect(await usdce.balanceOf(myAccount.address)).to.be.withinPercent(ONE_GRAND_USDCE, 1);
  });
});
